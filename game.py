...
if __name__ == "__main__":
    ...
    # cargando las serpientes salvadas
    ...
    pygame.display.set_caption('Generación : 0\t\t  Puntaje : 0')
    # semilla generada para que cada serpiente vea el mismo conjunto de alimentos para comparar el rendimiento
    seed = random.random()
    ...
    for saved_snake in snakes:
        ...
        while t_snake.isAlive():
            # Comprobación de pulsaciones de teclas y pulsaciones de botones de cierre y funcionalidad de pausa-continuación
            for event in pygame.event.get():
                ...
                if event.type == pygame.QUIT:
                    pygame.quit()
                    quit()
            # obteniendo resultado de la red neuronal
            ...
            # comprobando si hay bucles hechos por serpiente
            if t_snake.steps_taken > (len(t_snake.list)/5*100):
                ...
                checkloop = False
            if not alive:
                text = 'Generación : ' + str(generation+1) + '\t\t' + \
                    '  Puntaje : ' + str(len(t_snake.list)-1)+'\t [Muerta]'
                if t_snake.crash_wall and t_snake.crash_body:
                    print('Asesinada,', 'generación : ', generation+1, ' puntaje : ', len(t_snake.list)-1)
                elif t_snake.crash_wall and not t_snake.crash_body:
                    print('se estrelló contra la pared,', 'generación : ',
                          generation+1, ' puntaje : ', len(t_snake.list)-1)
                else:
                    print('se estrelló contra el cuerpo,', 'generación : ',
                          generation+1, ' puntaje : ', len(t_snake.list)-1)
            ...
            pygame.display.update()
            text = 'Generación : ' + str(generation+1) + '\t\t' + \
                '  Puntaje : ' + str(len(t_snake.list)-1)+'\t [Presione <q> para matar a la serpiente]'
            pygame.display.set_caption(text)
            pygame.display.update()
            time.sleep(0.03)
        generation += 1
